  ...
  const handleSubmit = async (status: 'DRAFT' | 'PUBLISHED') => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || error.message || `Failed to update blog (${response.status})`);
      }
      ...
      // Invalidate related blog caches (detail and my-blogs) so UI reflects update
      try {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              <Link href="/Doctor/blogs/my-blogs" className="text-gray-600 dark:text-gray-400 hover:text-teal-600 dark:hover:text-teal-400">
                    ...
                    {uploadProgress.status !== 'idle' && (
                      <div className="absolute left-4 right-4 bottom-3">
                        <LinearProgress
                          ...
                          onCancel={() => uploadProgress.reset()}
                        />
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-lg p-8 text-center hover:border-teal-500 dark:hover:border-teal-400 transition-colors cursor-pointer"
                    onClick={() => fileInputRef.current?.click()}
                  >
                    {uploadingImage ? (
                      <Loader2 className="w-12 h-12 mx-auto text-teal-600 dark:text-teal-400 animate-spin mb-3" />
                    ) : (
                      <ImageIcon className="w-12 h-12 mx-auto text-gray-400 dark:text-gray-500 mb-3" />
                    )}
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                      {uploadingImage ? 'Uploading...' : 'Click to select a cover image (upload happens on submit)'}
                    </p>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6 border border-gray-200 dark:border-slate-700">
                ...
                <div className="flex gap-2 mb-3">
                  <input
                    ...
                    className="flex-1 px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-white dark:bg-slate-700 text-gray-900 dark:text-white text-sm"
                  />
                  <button
                    onClick={addTag}
