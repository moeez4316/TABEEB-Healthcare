...
export const updateBlog = async (req: Request, res: Response) => {
  try {
    ...
    // If a new coverImagePublicId is provided, verify ownership and delete the old image
    if (data.coverImagePublicId !== undefined && data.coverImagePublicId) {
      // Verify ownership using existing blog's doctorUid
      const existing = await prisma.blog.findUnique({ where: { id }, select: { coverImagePublicId: true, doctorUid: true } });
      const ownerId = existing?.doctorUid || userUid || '';

      const { verifyPublicIdOwnership, buildCloudinaryUrl, deleteFromCloudinary } = await import('../services/uploadService');

      if (!verifyPublicIdOwnership(data.coverImagePublicId, ownerId, 'blog-image')) {
        return res.status(403).json({ error: 'Invalid cover image. Please upload again.' });
      }

      if (existing?.coverImagePublicId && existing.coverImagePublicId !== data.coverImagePublicId) {
        try {
          await deleteFromCloudinary(existing.coverImagePublicId);
        } catch (err) {
          console.warn('Failed to delete old blog cover image:', err);
        }
      }

      updateData.coverImageUrl = data.coverImageUrl || buildCloudinaryUrl(data.coverImagePublicId, 'image');
    }
